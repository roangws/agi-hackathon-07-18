...
| 3 | 0:35–1:05 | Terminal: `curl -X POST localhost:8099/mesh/kill -d '{"agent":"<working agent>"}'` (or press K). Node greys out. | "Now the moment that matters. I'm killing this agent — a real SIGKILL on a real OS process, mid-discharge. Its heartbeat lapses… the handoff is orphaned…" |
| 4 | 1:05–1:25 | Rescue: idle agent claims, HANDOFF NEVER DROPPED toast with bookmark | "…and an idle agent wins an atomic anycast race, replays the log, and resumes at the exact bookmark. No step repeated, none lost. We ship the test that proves it." |
